...
module EpisodeInfo
  ...
  region: #{config.AWS_REGION}"
    ...
    def perform(_sqs_msg, request)
      ...
      job.report_each_second(7) { PodcastInfoMonitor.starting_percent }
      result = TranSound::Podcast::EpisodeMapper.new(job.token).find('episodes', job.id, 'TW')
      job.report_each_second(7) { PodcastInfoMonitor.mapper_done }
      TranSound::Repository::For.entity(result).create(result)
      job.report_each_second(12) { PodcastInfoMonitor.finished_percent }
